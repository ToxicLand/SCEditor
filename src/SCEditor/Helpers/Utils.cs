...
namespace SCEditor.Helpers
{
    public static class Utils
    {
        ...
        public static int[] Solve32X32Blocks(int width, int height, int[] pixelArrayOld)
        {
            var modWidth = width % 32;
            var timeWidth = (width - modWidth) / 32;
            ...
            for (var timeH = 0; timeH < timeHeight + 1; timeH++)
            {
                ...
                for (var positionX = 0; positionX < 32; positionX++)
                {
                    offsetX = time * 32;
                    offsetY = timeH * 32;
                    pixels[(positionY + offsetY) * width + positionX + offsetX] = GetColorFromPxArray(pixelArrayOld, width, height);
                }
        ...
        public static int GetColorFromPxArray(int[] pixelArrayOLD, int width, int height)
        {
            //if (CountGCFPXA > width)
            if (_countGcfpxa > width - 1)
            {
                _countGcfpxa = 0;
                _countGcfpxaH += 1;
            }
            //Debug.Print($"test = {CountGCFPXA}-h{CountGCFPXA_H}");
            var goodColor = pixelArrayOLD[_countGcfpxaH * width + _countGcfpxa];
            //Color.PaleGreen;//pixelArrayOLD[CountGCFPXA_H,CountGCFPXA];
            _countGcfpxa += 1;
            return goodColor;
        }
